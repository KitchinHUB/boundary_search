...
class sample:
    ...
    def acquisition(self, r=0.1):
        ...
        for i in self.bound:
            curvePoint = i
            point_bool_in_rad = np.sum((self.X - curvePoint)**2,
                                       axis=1) <= r**2
            ...
            cat_inrad = self.cat[point_bool_in_rad]    # get the classification of these points
            ...
            radius_close.append(np.min(np.sum((self.X - curvePoint)**2,
                                              axis=1)))

        # aggregate all info into a 2D array
        bPoint = np.vstack((self.bound.T[0], self.bound.T[1], density,
                            np.abs(density_class), radius_close)).T
        ...
        class_density_ind = np.where(lowd_bPoint[:, 3] ==
                                     np.max(np.abs(lowd_bPoint[:, 3])))
    ...
    def first_sample(self, n=8, dis=0.4, w=10, rad_acq=0.1, seed=42):
        ...
        self.center_points = np.append(self.center_points,
                                       [self.acquisition(r=rad_acq)[0]], axis=0)
    ...
    def sample_simulation(self, n1=8, dis1=0.4, n2=4, dis2=0.2,
                          w=10, min_points=3, tol=0.0001, conv_trials=2,
                          domain_step=0.1, rad_acq=0.1, rad_den=0.1):
