...
class sample:
    ...
    def iter_sample(self, w=10, min_points=1, r_acq=0.2, tol=0.0001,
        ...
        while len(X_next) > 0:
            bound_subset = self.bound[np.array(self.compute_density(self.X,
                                                                    self.bound,
                                                                    r=r_acq))
                                      < min_points]
     
            X_next = self.acquisition(bound_subset, r=r_acq,
                                      min_points=min_points, samples=samples)
    ...
    def sample_simulation(self, ni=16, x1max=2, x2max=5, w=10, min_points=1,
                          r_acq=0.2, tol=0.0001, conv_trials=2, samples=8):
        ...
        self.iter_sample(w=w, min_points=min_points, r_acq=r_acq,
                         tol=tol, conv_trials=conv_trials, samples=samples)
