...
class PathSampler:
    ...
    def acquisition(self, center, bound, step=0.2):
        ...
        disbound = np.sum((bound[:boundind] - bound[boundind]) ** 2, axis=1) ** 0.5
    ...
    def get_bound(self, X, cat):
        ...
        grid_x, grid_y = np.mgrid[
            X1.min() - self.scale : X1.max() + self.scale : 100j,
    ...
    def iter_sample(
        ...
        while ((conv_ratio > atol).all() | (startend > centol)) & (len(self.X) < 2000):
            ...
            if ran_sam:
                ...
                X_next = c1 + dis1 * np.array(
                    [[-uvec[1], uvec[0]], [uvec[1], -uvec[0]]]
                )
